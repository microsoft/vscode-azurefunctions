...
suite('Function App Operations', function (this: Mocha.Suite): void {
    ...
    test('Create - Advanced', async () => {
        const testInputs: (string | RegExp)[] = [appName, 'Flex Consumption', location, /\.net/i, 'Windows', '$(plus) Create new resource group', rgName, '$(plus) Create new storage account', saName, '$(plus) Create new Application Insights resource', aiName];
    ...
    test('Create - Advanced - Existing RG/SA/AI', async () => {
        const testInputs: (string | RegExp)[] = [app2Name, 'Flex Consumption', location, /\.net/i, 'Windows', rgName, saName, aiName];
        await runWithTestActionContext('createFunctionAppAdvanced', async context => {
            await context.ui.runWithInputs(testInputs, async () => {
                await createFunctionAppAdvanced(context);
            });
