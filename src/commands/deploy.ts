...
export async function deploy(telemetryProperties: TelemetryProperties, target?: vscode.Uri | string | IAzureParentNode<FunctionAppTreeItem>, functionAppId?: string | {}): Promise<void> {
    ...
    const onNodeCreatedFromQuickPickDisposable: vscode.Disposable = ext.tree.onNodeCreate((newNode: IAzureNode<FunctionAppTreeItem>) => {
    ...
    try {
        if (!node) {
            if (!functionAppId || typeof functionAppId !== 'string') {
                node = <IAzureParentNode<FunctionAppTreeItem>>await ext.tree.showNodePicker(FunctionAppTreeItem.contextValue);
            } else {
                const functionAppNode: IAzureNode | undefined = await ext.tree.findNode(functionAppId);
                if (functionAppNode) {
