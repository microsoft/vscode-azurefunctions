...
export class FunctionAppListStep extends AzureWizardPromptStep<IFuncDeployContext> {
    ...
    private async getPicks(context: IFuncDeployContext): Promise<IAzureQuickPickItem<Site | undefined>[]> {
        ...
        const qp: IAzureQuickPickItem<Site | undefined>[] = sites.filter(s => !(s.kind?.includes('container')) && !!s.kind?.includes('functionapp')).map(fa => { // include only function apps but exclude containerized function apps
            return {
