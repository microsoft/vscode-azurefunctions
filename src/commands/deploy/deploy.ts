...
import { CodeAction, deploySubpathSetting, hostFileName, ProjectLanguage, remoteBuildSetting, ScmType, stackUpgradeLearnMoreLink, StorageType } from '../../constants';
...
async function deploy(actionContext: IActionContext, arg1: vscode.Uri | string | SlotTreeItem | undefined, arg2: string | {} | undefined, _expectedContextValue?: string | RegExp): Promise<void> {
    ...
    const durableStorageType: StorageType | undefined = await durableUtils.getStorageTypeFromWorkspace(language, context.projectPath);
    context.telemetry.properties.durableStorageType = durableStorageType;

    if (durableStorageType === StorageType.SQL && isFlexConsumption) {
...
async function updateWorkerProcessTo64BitIfRequired(context: IDeployContext, siteConfig: SiteConfigResource, site: ParsedSite, language: ProjectLanguage, durableStorageType: StorageType | undefined): Promise<void> {
    const client = await site.createClient(context);
    const config: SiteConfigResource = {
        use32BitWorkerProcess: false
    };

    if (durableStorageType === StorageType.Netherite) {
