...
export class DotnetTemplateProvider extends TemplateProviderBase {
    ...
    public async getCachedTemplates(context: IActionContext): Promise<ITemplates | undefined> {
        ...
        const projectFilePath: string = getDotnetProjectTemplatePath(context, this.version, projKey, this._overrideExtVariables);
        const itemFilePath: string = getDotnetItemTemplatePath(context, this.version, projKey, this._overrideExtVariables);
        ...
        if (cachedDotnetTemplates) {
            return await parseDotnetTemplates(context, cachedDotnetTemplates, this.version);
        } else {
            return undefined;
        }
    }
    ...
    public async getLatestTemplateVersion(context: IActionContext): Promise<string> {
        ...
        const templateVersion = await cliFeedUtils.getLatestVersion(context, this.version, this._overrideExtVariables);
        let netRelease = await this.getNetRelease(context, projKey, templateVersion);
        if (netRelease) {
            return templateVersion;
        } else {
            const templateVersions = await cliFeedUtils.getSortedVersions(context, this.version, this._overrideExtVariables);
            for (const newTemplateVersion of templateVersions) {
                try {
                    netRelease = await this.getNetRelease(context, projKey, newTemplateVersion);
                    if (netRelease) {
                        const latestFuncRelease = await cliFeedUtils.getRelease(context, templateVersion, this._overrideExtVariables);
                        ...
                        this._ext.outputChannel.appendLog(warning);
            ...
            for (const newFuncVersion of Object.values(FuncVersion)) {
                try {
                    if (newFuncVersion !== this.version) {
                        const newTemplateVersion = await cliFeedUtils.getLatestVersion(context, newFuncVersion, this._overrideExtVariables);
                        netRelease = await this.getNetRelease(context, projKey, newTemplateVersion);
                        if (netRelease) {
    ...
    private async getNetRelease(context: IActionContext, projKey: string, templateVersion: string): Promise<cliFeedUtils.IWorkerRuntime | undefined> {
        const funcRelease: cliFeedUtils.IRelease = await cliFeedUtils.getRelease(context, templateVersion, this._overrideExtVariables);
    ...
    public async getBackupTemplates(context: IActionContext): Promise<ITemplates> {
        const projKey = await this.getProjKey(context);
        const files: string[] = [getDotnetProjectTemplatePath(context, this.version, projKey, this._overrideExtVariables), getDotnetItemTemplatePath(context, this.version, projKey, this._overrideExtVariables)];
    ...
    public async clearCachedTemplates(context: IActionContext): Promise<void> {
        ...
        await AzExtFsExtra.deleteResource(getDotnetTemplateDir(context, this.version, projKey, this._overrideExtVariables), { recursive: true });
    }

    private async parseTemplates(context: IActionContext, projKey: string): Promise<ITemplates> {
        this._rawTemplates = parseJson(await executeDotnetTemplateCommand(context, { version: this.version, projTemplateKey: projKey, workingDirectory: undefined, operation: 'list', overrideExtVariables: this._overrideExtVariables }));
        return await parseDotnetTemplates(context, this._rawTemplates, this.version);
    }
