...
export function getScriptVerifiedTemplateIds(version: string): (string | RegExp)[] {
    ...
    if (version === FuncVersion.v1) {
        ...
        const regExps = verifiedTemplateIds.map(t => new RegExp(`^${t}-(JavaScript(-4.x)?|TypeScript(-4.x)?|Python|Ballerina|PowerShell|Custom|Python-Preview|Python-Preview-Append)$`, 'i'));
