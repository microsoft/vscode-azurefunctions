...
export namespace ballerinaUtils {
    ...
    export async function executeInit(context: IProjectWizardContext): Promise<void> {
        try {
            await cpUtils.executeCommand(undefined, context.projectPath, ballerinaCommand, composeArgs(withArg('new', '.'))());
    ...
    async function checkVersionByCLI(): Promise<number | undefined> {
        const result: cpUtils.ICommandResult = await cpUtils.tryExecuteCommand(undefined, undefined, "bal", composeArgs(withArg('--version'))());
