...
export namespace feedUtils {
     ...
     */
    export async function getJsonFeed<T extends {}>(context: IActionContext, url: string, overrideExtVariables?: IExtensionVariables): Promise<T> {
        let cachedFeed: ICachedFeed | undefined = cachedFeeds.get(url);
        if (!cachedFeed || Date.now() > cachedFeed.nextRefreshTime) {
            const response = await requestUtils.sendRequestWithExtTimeout(context, { method: 'GET', url }, overrideExtVariables);
